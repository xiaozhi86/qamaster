...
| 纯内联文本 | 走旧路径，不变 |
| 多 `@file` 里混有非文件 token（如内联补充说明） | `_parse_input_docs` 返回 `(None,False)` → 退回旧单输入路径（v1 不破坏、也不支持混排，列为后续增强） |
| 路径含空格 | 空白拆分会把一个路径切成两段 → 判定非文件 → 退回单输入（与现状 `os.path.isfile(整串)` 限制一致） |
...
1. **需求 vs 设计一致性核对**：多文档合并后，Phase 0 清单块可追加一句提示，让专家团额外核对「原始需求表述的规则/口径，与设计文档的技术/接口/数据细节是否冲突、是否遗漏」。纯提示语增强，不动流程。建议 v1 跑通主链路后再评估。
2. **混排输入**：多 `@file` 与内联补充说明混排时，当前退回单输入；后续可升级 `_parse_input_docs` 支持「文件 token + 内联片段」混合。
3. **路径含空格**：改用 `shlex.split` 或约定引号包裹后拆分，支持含空格路径的多文件。
