...
def test_requirement_review_multi_docs():
    ...
    验证：①单文件/单内联/空 → (None,False)（走旧单输入）；②两文件 → (entries,True) 顺序正确 + @ 前缀剥离；
    ③含非文件 token（内联补充说明）混排 → (entries,True) 含 ("text",片段)；④引号包裹解析（整串/逐文件/含空格路径）；
    ⑤无 INPUTS_<id>.md → Phase 0 卡片不追加清单块；⑥有 INPUTS_<id>.md → 卡片含多文档清单 + 需求vs设计一致性核对指令。
    """
    ...
    rr_out = "requirement-review-out"
    try:
        # 造真实文件（含一个带空格文件名）+ 一个不存在路径
        f_main = os.path.join(workdir, "需求.md")
        f_d1 = os.path.join(workdir, "设计A.md")
        f_d2 = os.path.join(workdir, "设计B.md")
        f_space = os.path.join(workdir, "设计 文档.md")   # 含空格路径
        for f in (f_main, f_d1, f_d2, f_space):
            with open(f, "w", encoding="utf-8") as fh:
                fh.write("# 标题\n\n正文。\n")

        # ① 单文件 / 单内联 / 空 → (None, False)
        entries, multi = rt._parse_input_docs(f_main)
        check("单文件 → (None,False)", (entries is None and multi is False), "entries=%s multi=%s" % (entries, multi))
        entries, multi = rt._parse_input_docs("用户在购物车点击提交订单")
        check("单内联 → (None,False)", (entries is None and multi is False), "entries=%s multi=%s" % (entries, multi))
        entries, multi = rt._parse_input_docs("")
        check("空输入 → (None,False)", (entries is None and multi is False))

        # ② 两文件 → (entries,True) 顺序正确 + @ 前缀剥离
        entries, multi = rt._parse_input_docs("@%s @%s" % (f_main, f_d1))
        check("两文件 → (entries,True)", multi is True and entries == [("file", f_main), ("file", f_d1)],
              "entries=%s" % entries)

        # ③ 含非文件 token：单文件+内联 → 仍退回 (None,False)（文件数<2）；两文件+内联 → (entries,True) 混排
        entries, multi = rt._parse_input_docs("%s 补充说明" % f_main)
        check("单文件+内联 → (None,False)", (entries is None and multi is False), "entries=%s" % entries)
        entries, multi = rt._parse_input_docs("%s %s" % (f_main, os.path.join(workdir, "不存在.md")))
        check("含不存在文件 → (None,False)", (entries is None and multi is False), "entries=%s" % entries)
        entries, multi = rt._parse_input_docs("%s @%s 补充说明文字" % (f_main, f_d1))
        check("两文件+内联混排 → (entries,True)",
              multi is True and entries == [("file", f_main), ("file", f_d1), ("text", "补充说明文字")],
              "entries=%s" % entries)

        # ④ 引号包裹解析：整串外层引号 / 逐文件引号 / 含空格路径
        entries, multi = rt._parse_input_docs('"%s %s"' % (f_main, f_d2))
        check("整串引号包裹两文件 → (entries,True)", multi is True and entries == [("file", f_main), ("file", f_d2)],
              "entries=%s" % entries)
        entries, multi = rt._parse_input_docs('"%s" "%s"' % (f_main, f_d2))
        check("逐文件引号包裹 → (entries,True)", multi is True and entries == [("file", f_main), ("file", f_d2)],
              "entries=%s" % entries)
        entries, multi = rt._parse_input_docs('"%s" "%s"' % (f_main, f_space))
        check("含空格路径（引号包裹）→ (entries,True)",
              multi is True and entries == [("file", f_main), ("file", f_space)], "entries=%s" % entries)
        entries, multi = rt._parse_input_docs('@"%s" @"%s"' % (f_main, f_space))
        check("含空格路径（@+引号包裹）→ (entries,True)",
              multi is True and entries == [("file", f_main), ("file", f_space)], "entries=%s" % entries)
        ...
        # ⑥ 有 INPUTS_<id>.md → Phase 0 卡片含多文档清单 + 一致性核对指令
        rt._write_inputs_manifest(workdir, spec, rid, [("file", f_main), ("file", f_d1), ("file", f_d2), ("text", "补充说明文字")])
        block = spec.extra_card_text(0, st)
        check("有清单文件 → 注入多文档综合评审块", "多文档综合评审" in block, block[:200])
        check("清单块含主需求文档 + 设计文档 + 补充说明", "主需求文档" in block and "设计文档" in block and "补充说明" in block, block[:400])
        check("清单块含合并指令（REQ_ 落盘）", "REQ_<需求标识>.md" in block, block[:300])
        check("清单块含需求vs设计一致性核对", "需求 vs 设计一致性核对" in block, block[:400])
