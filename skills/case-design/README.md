...
> 更新：2026-08-19 · 版本变更明细见 `CHANGELOG.md`
...
**UI 类测试子维度完备**：UI 测试锚定"界面结构是否正确承载业务规则/数据/权限/状态"（查询条件项、列表字段映射/分页排序/四态区分/脱敏、表单联动回显、按业务态启用按钮），不测元素存在/框架渲染；8 子维 + 边界表（不与其他维度重复）+ 适度性校准（按风险分级，防爆炸）。
...
> **设计文档输入（可选通道）**：用 `<<<设计文档开始>>>` / `<<<设计文档结束>>>` 包裹，与需求文档并列。提供后：① 第0阶段整文落盘 `case-design-out/DESIGN_<需求标识>.md`（非 .md 文件路径须经 `scripts/extract_doc.py --kind design` 落盘）；② 第2阶段覆盖矩阵新增"8.11 设计文档测试要点覆盖"维度；③ 第7阶段测试点建模须引用设计文档测试要点；④ 第8/10/13阶段 `#8-H` 反向设计文档测试要点追溯（漏覆盖即 exit=1）。设计文档无任何可追溯章节时，须从正文/字段映射/异常处理补建 `## 测试要点` 章节，不得 SKIP。
...
- **UI 类覆盖**：UI 测试锚定"界面承载业务规则/数据/权限/状态"（查询/列表/表单/按钮/权限UI 8 子维），不测元素存在/框架渲染；判定线 = Then 断言业务可观测 vs 元素存在性；按风险分级（P0/P1全量、P2采样、P3最小），防爆炸。
