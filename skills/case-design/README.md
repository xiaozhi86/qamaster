...
2. **澄清**：扫描缺口，输出【待确认问题列表】。若有 **P0/P1** 未确认项，会**暂停等回复**；请逐条回答（可批量，如"Q1 按方案A，Q2 按方案B"）。答复会落盘到 `case-design-out/Clarification_Ledger_<需求标识>.md`，后续不再重复提问。
...
│   ├── selfcheck.md            # 第11阶段：14项自查（含断言完整性/对抗生成遍）+ 自修/阻断决策
