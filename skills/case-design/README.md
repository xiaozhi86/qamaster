...
**每个阶段都有阶段出口校验**：第3/5/8阶段在写前于**内存内**跑机器检查（`verify_cases.py run_inmemory`，≤2轮有界自修），第5/8阶段另加**有界对抗式 critique 循环**（≤2轮，查机器查不出的"P0 漏标""漏测用例"），第11阶段写前 LLM 自查（≤3轮）+ 第13阶段写后回读。机器判定与主观对抗分层、各计数器独立不嵌套——缺陷在阶段出口就地拦住，不再聚合到写后才反向捕获。
...
3. **建模/风险/方法/测试点/用例生成**：默认不展示过程（仅调试模式可见）。**各阶段出口有校验**：第3阶段校验规则来源、第5阶段校验风险来源并跑 P0 漏标 critique、第8阶段生成完在内存内跑 `verify_cases.py` 全量机器检查（≤2轮有界自修）+ 检查14对抗遍 critique，确保进入去重/覆盖率/自查阶段的用例集已通过机器校验。
...
- **阶段出口机器校验**：第3/5/8阶段在写前于内存内跑 `verify_cases.py`（≤2轮有界自修，零临时文件），缺陷在阶段出口就地拦住，不拖到写后才发现；第5/8阶段另跑有界 critique 循环（≤2轮）查机器查不出的漏标/漏测。
