---
name: case-design
description: Use when 根据需求文档、原型图、业务上下文设计测试用例，或将已有Markdown测试用例转为Excel；用户提到测试用例设计、用例生成、需求转用例、用例转Excel、测试覆盖分析、测试点挖掘、风险用例识别时。涵盖需求澄清、规格建模、风险优先、方法动态匹配、去重与过度设计剔除、Markdown与Excel双格式按需输出。
...
不允许脑补需求/表名/字段名/缓存Key；不允许生成伪测试/重复测试；不允许输出模糊断言；不允许跳过澄清环节；不允许重复询问澄清台账中已解决的问题；不允许增量写入文件；不允许跳过人工审核门禁；不允许遗留临时本地文件。
...
> **可选通道的流向**：技术实现摘要 → 第4阶段 SDD（State/Exception/契约事实校对）+ 第8阶段测试数据 + 检查9（存储可断言，见 references/selfcheck.md）；业务规则与历史缺陷 → 第3阶段规则建模 + 第5阶段业务领域风险；历史缺陷摘要 → 第5阶段缺陷种子测试点（强制覆盖）。三者均可选，未提供时 skill 退回当前"自然语言兜底+澄清提问"行为，不阻断。
...
10. Coverage Validation（覆盖率校验 + 反向需求追溯 #4，详见 `references/dedup_coverage.md`）
11. Pre-Output Self-Check（输出前自查，14项：含检查13 断言完整性、检查14 对抗生成遍，详见 `references/selfcheck.md`，**内存内零文件操作**）
...
**默认按需求规模自动分级**（第0阶段判定，见 `references/dedup_coverage.md` ch26 与 `references/phase0_manifest.md`）：重型（新建 P0/核心链路）走完整 15 步；中型（已有模块新增 P1）合并建模、裁剪部分阶段；轻型（字段/文案/低风险 P2P3）跳过完整规格建模与覆盖矩阵。流程深度（执行哪些阶段）与运行模式（人工介入程度，见 6.5）是两个正交维度：用户显式声明"连跑/轻量"时同时设定运行模式与流程深度（连跑→中型、轻量→轻型）；未声明则流程深度按规模自动分级、运行模式默认完整。
...
**核心要点速记**：
* 本轮全部用例先在上下文（内存）完整生成 + 完成第22章全部自查（14项，含检查13 断言完整性、检查14 对抗生成遍）+ 自修循环（最多3轮，全部内存内），自查自修是内存操作不是文件编辑
...
1. 输出【待确认问题列表】（有问题则停，等用户回复；问题按角色路由标注 @开发/@产品/@业务）
2. 输出【测试需求分析结果】（仅调试模式可见）
3. 输出【测试点清单】（仅调试模式可见）
4. 对话中展示用例紧凑投影+覆盖矩阵（只展示，不写文件，禁止此时发文件写入调用；明细15列见第6步文件）
5. 内存内自查自修（14项，含检查13 断言完整性、检查14 对抗生成遍，零文件操作）+ 写前规模评估（输出 token 预算，决定单文件/PART拆/展示与写入是否分响应）
...
要求：连跑跳过等待仍须输出摘要；P0/P1 阻断项须显式列出；Excel/知识总结/清理须如实填写，未通过不得声明已生成。**脚本校验摘要**四项数值须从 `verify_cases.py` 回读输出的对应行直接取（检查13 断言完整性 / 检查9增强 存储schema交叉 / 风险来源 / 反向需求追溯 #4），禁止凭印象手填；跳过项填"跳过"并注明原因（如未传需求文档、未提供技术实现摘要）。**拆 PART 自动续跑时**：中间 PART 落盘后交付摘要"下一步建议"填"自动续跑下一 PART"，仅末 PART 填"统一审核全部文件"；审核状态以全部 PART 聚合填写。
...
| 第9-10阶段 | `references/dedup_coverage.md` | 去重、覆盖率控制、停止条件、需求规模分级（原 ch16/17/18/26） |
| 第11阶段 | `references/selfcheck.md` | 14项自查（含检查13 断言完整性、检查14 对抗生成遍）、自修vs阻断决策、门禁失败处理（原 ch22） |
...
> 各类禁止性规则已分布于对应 references 的"禁止/强制"节，此处不重复罗列；执行时以各阶段 ref 为唯一细则来源。核心红线（违反任一即判定输出不合格，见 3.1）：不脑补需求/表名/字段/缓存Key、不生成伪测试/重复测试、不输出模糊断言、不跳过澄清、不重复询问台账已解决项、不增量写入文件、不跳过人工审核门禁、不遗留临时文件。
