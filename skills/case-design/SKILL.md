...
| **Phase 8 用例生成** | **完整 15 列用例表**（首列"用例ID"，与最终 TestCases.md 同结构；含追溯性 section 规则建模/风险清单/测试点清单）。**顺序强制**（v0.11.0·RC3 修复）：追溯性 section 须在用例表**之前**，顺序为 规则建模 → 风险清单 → 测试点清单 → 用例表，与 `references/output_write.md` L199 一致；顺序写反（用例表在前）由 `verify_cases.py` 的 `check_section_order` 硬阻断（Phase 8 gate FAIL），勿靠自觉。| 覆盖预检摘要 |
