...
> 更新：2026-08-25 · 版本变更明细见 `CHANGELOG.md`
> Runtime 受控流程支持多需求并行评审（8 阶段状态机，见 §7）；契约卡末尾按需追加 `##CONTEXT_BUDGET##` 上下文建议（Phase 5/6 重输出点提示 `/compact`），并新增 `context` 只读命令查工作集估算与累计 token 足迹；Phase 0 专家团动态路由（`config/agents.json` + `contains` 门禁），路由升级为三级裁决 + 存疑即启用（`co_trigger` 连带 + 语义兜底）；支持多文档综合评审（1 主需求 + N 设计文档，见 §6.2），含空格路径引号包裹 + 混排输入 + 需求 vs 设计一致性核对
...
- **多文档综合评审**：可一次提供多份文件 `@主需求.md @设计文档A.md @设计文档B.md`——**第一个文件为主需求文档**（决定需求标识、专家团信号词路由、MANIFEST 需求文档列），其余按序作为设计文档；Runtime 落盘 `INPUTS_<需求标识>.md` 清单，第0阶段把多份文档按序合并为一份 `REQ_<需求标识>.md` 评审语料（主需求在前，各设计文档加 `## 输入文档：<文件名>` 分节），后续评审/重构统一综合多份文档、输出一份 `ReviewedReq_<需求标识>.md`。
- **多文档增强**：①含空格路径——用引号包裹即可 `@"主需求.md" @"设计 文档A.md"`（`shlex` 按引号拆分，保留 Windows 反斜杠）；②混排输入——文件 token 与内联补充说明可混排（如 `@主需求.md @设计A.md 补充：支付接口需幂等`），内联片段按出现位置追加为「补充说明 N」分节；③需求 vs 设计一致性核对——评审专家团逐条对照主需求文档的业务规则/数据口径/状态流转/异常边界，与各设计文档的技术/接口/数据结构细节，找出冲突与遗漏并统一回填。
...
SKILL.md 的「并行评审 + 汇总仲裁」9 阶段（0-9）已压缩为 **8 个受控阶段（0-7）**，由 `runtime/requirement_review_phases.py` 驱动（与 case-design 同走通用 workflow 引擎）：
...
| 文件 | 内容 |
|---|---|
| `requirement-review-out/REQ_<需求标识>.md` | 预处理后的需求文档纯文本（第0阶段落盘，评审的唯一文本输入基准；多文档评审时为合并后的综合语料） |
| `requirement-review-out/INPUTS_<需求标识>.md` | 多文档评审输入清单（仅多文档时出现；Runtime 落盘，模型只读；含主需求文档/设计文档 N/可选补充说明 N，支持含空格路径与混排输入） |
