...
2. 多文档综合评审（可选·当契约卡出现「多文档综合评审」清单块时）
   - 契约卡 Phase 0 注入「输入清单块」（INPUTS_<需求标识>.md）：第一份为主需求文档，其余为设计文档
   - 对清单中每份文件分别跑 `python skills/requirement-review/scripts/extract_text.py <文件> --json` 抽取纯文本
   - 按清单顺序合并落盘 requirement-review-out/REQ_<需求标识>.md：主需求文档在前，各设计文档按序追加，每份前加「## 输入文档：<文件名>」二级标题分节，保留各份内容原样不删减
