...
PHASES = [
    ...
    },
    {
        ...
        "objective": "沉淀规则建模清单，每条规则标注来源 [来源:需求文档<章节>/台账Q<n>/假设A<n>]，出口机器 gate 校验规则来源。",
        "allowed": ["建立规则建模清单（内存，后续沉淀为 TestCases .md 的 section）", "每条规则项标注来源"],
        "forbidden": ["无来源标记的规则项静默保留（须转问题/假设）", "脑补业务规则"],
        "produces": [],
        "exit_condition": "规则清单全部带来源标注（本阶段不写文件，由 Phase13 统一沉淀落盘）",
        "gate_checks": [],
    },
    {
    ...
    },
    {
        ...
        "objective": "三源共验产出 P0-P3 风险清单（含风险来源标注）；出口机器 gate + P0 漏标 critique 循环。",
        "allowed": ["产出风险清单（风险ID/等级/描述/模块/来源）", "技术隐含@开发/业务领域@业务/缺陷反哺来源的风险经台账角色确认（完整模式）"],
        "forbidden": ["P0 风险不显式列出", "跳过风险分析直接用例"],
        "produces": [],
        "exit_condition": "风险清单完整含来源，critique ≤2 轮无新增漏标",
        "gate_checks": [],
    },
    {
    ...
    },
    {
        ...
        "refs": ["references/coverage.md"],
        "objective": "产出测试点清单（测试点ID/场景类型/描述/模块），覆盖主流程/分支/异常/状态/权限/数据一致性/幂等/并发/UI结构。",
        "allowed": ["按 15.3 维度提取测试点", "测试点清单后续沉淀为 TestCases .md section"],
        "forbidden": ["遗漏高风险测试点", "测试点无场景类型标注"],
        "produces": [],
        "exit_condition": "测试点清单完成且覆盖高风险维度",
        "gate_checks": [],
    },
    {
        "id": 8, "name": "用例生成", "gate": "auto",
        "refs": ["references/modeling.md", "references/quality_rules.md"],
        "objective": "生成全部 Given/When/Then 用例（内存），遵守断言完整性/数据真实/存储保护/方法落地，关联规则列引用 R<n>/TP<n>/API<n>/假设A<n>。",
        "allowed": ["内存内生成全部用例（不落盘）", "断言可观测且状态变更类双重断言", "关联规则含 ID 引用实现追溯"],
        "forbidden": ["脑补无来源业务行为", "模糊断言", "杜撰存储信息", "边生成边写文件"],
        "produces": [],
        "exit_condition": "全部用例内存生成完毕（写前零文件操作）",
        "gate_checks": [],
    },
    {
    ...
    },
    {
        ...
        "refs": ["references/dedup_coverage.md"],
        "objective": "覆盖率校验 + #4 反向需求追溯 + #5 业务行为来源追溯（契约分支加 #6 接口追溯）；未覆盖/无来源项内存补齐或转问题/假设。",
        "allowed": ["按停止条件收敛（核心规则/高风险/关键状态/核心异常已覆盖即停）", "缺口转待确认问题/假设并回显清单"],
        "forbidden": ["为覆盖率而覆盖、无限扩展边缘场景", "REQ 缺失时宣称覆盖率全过"],
        "produces": [],
        "exit_condition": "覆盖缺口闭合或已登记假设",
        "gate_checks": [],
    },
    {
    ...
    },
    {
        ...
        "gate_checks": [
            ...
            {"kind": "script", "cmd": "python \"{skill_scripts}/verify_cases.py\" \"case-design-out/TestCases_{req_id}.md\" \"case-design-out/REQ_{req_id}.md\"", "label": "内容回读(verify_cases)"},
