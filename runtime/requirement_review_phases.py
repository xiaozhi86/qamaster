...
PHASES = [
    {
        ...
        "refs": [],
        "objective": "将用户需求文档（含图片/扫描件）经 extract_text.py 预处理为纯文本并落盘 requirement-review-out/REQ_<需求标识>.md（纯散文补 ## 二级标题分节）。",
        "allowed": [
        ...
        "forbidden": ["跳过需求文档落盘直接进入评审", "生成评审结论或最终需求文档"],
    ...
    },
    {
        ...
        "refs": [],
        "objective": "7 个专家 Agent（BA/PM/QA/Arch/UX/Risk/Dev）并行评审，各自输出 ✅/❌/⚠ 问题清单，落盘 requirement-review-out/ReviewIssues_<需求标识>.md。",
        "allowed": ["7 Agent 并行评审（各输出问题建议+依据）", "问题清单落盘 ReviewIssues_<需求标识>.md"],
        "forbidden": ["跳过任一 Agent 直接汇总", "评审结论与问题清单不落盘"],
    ...
    },
    {
        ...
        "refs": [],
        "objective": "Review Master 汇总 7 Agent 结果、去重，检测 Agent 间冲突（业务 vs 技术 / 体验 vs 风控），输出冲突清单与权衡推荐（内存，回填 ReviewIssues 清单）。",
        "allowed": ["汇总去重 7 Agent 问题", "输出【冲突清单】（冲突点/涉及 Agent/原因/推荐方案）"],
        "forbidden": ["遗漏 Agent 间冲突", "跳过去重导致问题清单重复"],
        "produces": [],
